...
class AppStore extends Component {
  state = {tabID: tabsList[0].tabId, inputVal: ''}

  tabClick = T_ID => {
    this.setState({tabID: T_ID})
  }
  ...
  render() {
    const {tabID, inputVal} = this.state
    const filteredList = appsList.filter(each => each.category === tabID)
    const searchValList = filteredList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(inputVal.toLowerCase()),
    )
    return (
